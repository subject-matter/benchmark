      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setCurrentProcessIndex(Number(entry.target.getAttribute('data-index')));
          }
        });
      },
      { threshold: 0.5 }
    ...
    <section className={`col-span-12 fade-in-section mt-[300px] `}>
      <div className="grid grid-cols-2 min-h-screen relative">
        <div className={`row-span-4 col-span-2 md:col-span-1`}>
          <div className="group">
            <div className={`hidden md:block bg-white z-[1] pb-10  mx-[10px] md:mx-5 col-span-2  md:col-span-1`}>
          ...
          {/* <div
            className={`numbers bg-white z-[2] border-t md:border-b border-dotted border-grey pt-3 pb-32 md:pb-24 mx-[10px] md:mx-5 col-span-2  md:col-span-1 mt-12   `}
          >
            <h4 className="md:pt-3 text-xs-medium pb-8 md:pb-0">Years of Experience</h4>
            <CountTrigger onEnter={() => handleEnterViewport(0)}>
              {countersOn[0] && (
                <CountUp start={0} end={25} duration={1} suffix="+" className="font-medium text-sm-2xl md:text-2xl" />
              )}
            </CountTrigger>
          </div>

          <div
            className={`awards z-[3] relative bg-white border-t border-dotted border-grey pt-3 pb-32 md:pb-24 mx-[10px] md:mx-5 col-span-2 md:col-span-1 ${
              isInView ? '' : ''
            } `}
          >
            <h4 className="md:pt-3 text-xs-medium pb-8 md:pb-0">Master Builders Awards</h4>
            <CountTrigger onEnter={() => handleEnterViewport(1)}>
              {countersOn[1] && (
                <CountUp
                  start={0}
        ...
        >
          <div className={`h-screen flex flex-col justify-end sticky top-0`}>
            ...
            <div className={`font-medium text-sm-xl md:text-xl text-wrap fade-in ${isVisible ? 'visible' : ''}`}>
              <h1 className="font-medium text-sm-xl md:text-xl mb-medium">
                {currentProcessIndex == processes.length - 1 ? '+' : currentProcessIndex + 1}
              </h1>
              <h1
        ...
        <div className="md:bg-black  col-span-2 row-start-1 md:col-start-2 row-span-5 relative order-1 ">
          <p className="block md:hidden text-black text-xs-medium md:py-5 p-[10px]">About Us</p>
          <div className="bg-black md:sticky top-0 z-[6]">
            <Image className=" object-cover" src={Photo} alt="Richard and Sam" width={2000} height={1000} />
          </div>
        </div>
